...
public class RTree implements Serializable{
    ...
    public Pair<INodo, INodo> linearSplit(int m, INodo n, IRectangulo r){
        ...
        if(n.esHoja()){
            ...
            nodo_der = new NodoHoja(popNextId(), n.popRectangulo(i_der));
        }
        else{
            nodo_izq= new NodoInterno(popNextId(),n.popRectangulo(i_izq));
            nodo_der=new NodoInterno(popNextId(), n.popRectangulo(i_der));
        }
        ...
        while(!n.isEmpty()){
            ...
            if(n.cantidadRectangulos() + nodo_izq.cantidadRectangulos() == m ){
                nodo_izq.appendRectangulo(n.popRectangulo(random_index));
            }
            else if(n.cantidadRectangulos() + nodo_der.cantidadRectangulos() == m ){
    ...
    //M: memoria principal
    public Pair<INodo, INodo> GreeneSplit(int M, INodo n, IRectangulo rec){
        INodo nodo_izq = null;
        INodo nodo_der = null;
        ...
        //se insertan
        if(n.esHoja()){
            nodo_izq = new NodoHoja(popNextId(), n.popRectangulo());
            int i = 1;
            while (i < M/2 -1){
                nodo_izq.appendRectangulo(n.popRectangulo());
                i++;
            ...
            nodo_izq= new NodoInterno(popNextId(),n.popRectangulo());
            int i = 1;
            while (i < M/2 -1){
                nodo_izq.appendRectangulo(n.popRectangulo());
                i++;
    ...
    public void sort(INodo n, int l, int r, boolean dir_corte){
        if (l < r) {
            ...
            int m = (l+r)/2;   
            sort(n, l, m, dir_corte); 
            sort(n , m+1, r, dir_corte); 
  
            // Merge the sorted halves 
            merge(n, l, m, r, dir_corte); 
        } 
    }
    // Mezcla dos listas 
    // primera lista L[l, ... ,m]
